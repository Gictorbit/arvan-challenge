...
BEGIN
    ...
    IF NOT FOUND THEN
        RETURN QUERY SELECT 'User not found', 0;
        RETURN;
    END IF;
    ...
    IF NOT FOUND THEN
        RETURN QUERY SELECT 'Gift code not found', 0;
        RETURN;
    END IF;

    -- Check if event is currently active
    IF NOW() < v_event.start_time OR NOW() > v_event.end_time THEN
        RETURN QUERY SELECT 'Gift code is not active', 0;
        RETURN;
    END IF;
    ...
    IF v_event.published = FALSE OR v_event.user_count >= v_event.max_users THEN
        RETURN QUERY SELECT 'Gift code not valid or already fully used', 0;
        RETURN;
    END IF;
    ...
    IF EXISTS (SELECT 1 FROM user_events WHERE user_id = v_user_id AND event_code = p_code) THEN
        RETURN QUERY SELECT 'User has already applied this gift code', 0;
        RETURN;
    END IF;
    ...
    RETURN QUERY SELECT 'Gift code applied successfully', new_balance;
EXCEPTION
    WHEN OTHERS THEN
        RETURN QUERY SELECT 'Error applying gift code', 0;
END;
$$ LANGUAGE plpgsql;
