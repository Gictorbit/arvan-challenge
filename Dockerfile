...
RUN --mount=type=cache,mode=0755,target=/go/pkg/mod CGO_ENABLED=0 GOOS=linux GOARCH=amd64 \
    go build -ldflags "-s -w" -a -installsuffix cgo -o ./bin/arvansrv ./cmd/...

ARG COMPRESS
RUN mkdir -p /final && \
    if [ "$COMPRESS" = "true" ] ;then upx --best --lzma -o /final/arvansrv ./bin/arvansrv ;else cp ./bin/arvansrv /final; fi
